...
[AddComponentMenu("UI/Extensions/FITextPic")]
[ExecuteInEditMode] // Needed for culling images that are not used //
//public class FITextPic : Text, IPointerClickHandler, IPointerExitHandler, IPointerEnterHandler, ISelectHandler
public class FITextPic : Text
{
	/// <summary>
	/// Image Pool
	/// </summary>
...
//	public Sprite[] inspectorIconList;
	List<Tuple<string,Sprite>> container{
		get{
			return spriteContainer.GetListOfSpriteForTextPic();
		}
	}
	public FISpriteData spriteContainer;
...
//		{
//			foreach (Sprite icon in container)
//			{
//				// Debug.Log(icon.sprite.name);
//				iconList.Add(icon.name, icon);
//			}
//		}
		Reset_m_HrefInfos ();
	}
	...
	protected void UpdateQuadImage()
	{
		...
		foreach (Match match in s_Regex.Matches(m_OutputText))
		{
			...
			if (img.sprite == null || img.sprite.name != spriteName)
			{
				...
				{
					foreach (var icon in container)
					{
						if (icon.Item1 == spriteName)
						{
							img.sprite = icon.Item2;
							break;
						}
					}
				}
	...
	protected string GetOutputText()
	{
		...
		if (container != null && container.Count > 0)
		{
			foreach (var icon in container)
			{
				if (icon.Item1 != null && icon.Item1 != "")
				{
					fixedString = fixedString.Replace(icon.Item1, "<quad name=" + icon.Item1 + " size=" + fontSize + " width=1 />");
				}
			}
		}
		int count = 0;
