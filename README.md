...
## 다른 문서 바로 가기
1. [소스 코드](AttachedFileInfo.md)

## Table of content.
1. 개요
1. 슈퍼마리오 모작(2009/05)
1. iVindictus(2010/02)
1. 한경M&M iPAD 디지털 잡지 솔루션(2010/11)
...
* 앱만드는것보다 데이터 수집하고 입력하는게 더 힘들었었다. 인터넷으로 검색이 안되는 데이터는 직접 플레이하면서 넣음. (덤으로 영어버전도 지원해서 노동은 2배이상)
...
### 세부내용
* 회사가 출간하는 잡지를 사람들이 안읽게 되어 많은 고민이 있었음.
* 잡지 컨텐츠를 모바일로 볼 수 있도록 하는게 주 목적.  
...
### 세부내용
* 스웨덴 게임에서 유행하던 Turf을 한국어로 바꾸어 한국에서 서비스.  
...
* 게임은 잘 런칭하였지만, 잦은 버그와 해킹시도때문에 사람이 많이 없어짐. 다행이 회사를 나온 이후에 *Soul of legend*를 기반으로 후속작 *Legend of master* 와 *덴마* 라는 게임도 출시. 지금은 잘 되고 있는 것 같음.
...
* 클라이언트 3명이서 프로토타이핑 하고 있었고,나도 같이 껴서 프로토타입 완성함.   
...
회사|(주)레드모바일
재직기간|2017년5월~재직중
작업인원|약50명(개발팀 이외 경영지원 및 인사팀 포함)
사용언어|C#
